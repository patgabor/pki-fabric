...
/// </summary>
public sealed class Sha2Helper : ISha2Helper
{
    /// <inheritdoc />
    public byte[] ComputeSha256(ReadOnlySpan<byte> bytes) => SHA256.HashData(bytes);
    /// <inheritdoc />
    public byte[] ComputeSha384(ReadOnlySpan<byte> bytes) => SHA384.HashData(bytes);
    /// <inheritdoc />
    public byte[] ComputeSha512(ReadOnlySpan<byte> bytes) => SHA512.HashData(bytes);
}
